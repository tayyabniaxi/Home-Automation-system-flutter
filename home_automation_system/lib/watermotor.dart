...
class Watermotor extends StatefulWidget {
  @override
  _WatermotorState createState() => _WatermotorState();
}

class _WatermotorState extends State<Watermotor> {
...
//this function calls when press on mic icon
    ...
    speech.listen(
      onResult: (result) {
        setState(() {
          ...
          if (transcription == 'turn off motor') {
            fanState = 0;
            isValid = true;
          } else if (transcription == 'turn on motor') {
            fanState = 1;
            isValid = true;
          } else {
            isValid = false;
  ...
  void handleSwitch() {
    ...
    if (fanState == 1) {
      espServices.turnOnLed('motor');
    } else {
      espServices.turnOffLed('motor');
    }
  }
